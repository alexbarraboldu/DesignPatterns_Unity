...
public class SplineComponent : MonoBehaviour, ISpline
{
	...
	/// <summary>
	/// Index is used to provide uniformed point searching.
	/// </summary>
	SplineIndex uniformedIndex;
	SplineIndex Index
	{
		get
		{
			if (uniformedIndex == null) uniformedIndex = new SplineIndex(this);
			return uniformedIndex;
		}
	}
	...
	public Vector3 GetDistance(float distance)
	{
		...
		return uniformedIndex.GetPoint(distance / length.Value);
	}
	...
	{
		if (uniformedIndex != null) uniformedIndex.ReIndex();
	}
}
